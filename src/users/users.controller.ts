import { Controller, Get, UseGuards, Request, Post, Body, Put, Param, HttpStatus, HttpCode } from '@nestjs/common';
...
export class UsersController {
  ...
  @ApiResponse({ status: 201, description: 'Usuário criado com sucesso', type: RegisterOrUpdateUserResponseDto })
  @ApiResponse({ status: 400, description: 'Dados inválidos ou usuário já existe' })
  @ApiResponse({ status: 401, description: 'Não autorizado' })
  async registerAdminUser(@Body() registerAdminUserDto: RegisterAdminUserDto): Promise<RegisterOrUpdateUserResponseDto> {
  ...
  @ApiResponse({ status: 200, description: 'Usuário atualizado com sucesso', type: RegisterOrUpdateUserResponseDto })
  @ApiResponse({ status: 400, description: 'ID inválido ou dados inválidos' })
  @ApiResponse({ status: 404, description: 'Usuário não encontrado' })
  @ApiResponse({ status: 401, description: 'Não autorizado' })
  async updateAdminUser(@Param() params: FindUserParamDto, @Body() updateAdminUserDto: UpdateAdminUserDto): Promise<RegisterOrUpdateUserResponseDto> {
    const { id } = params;
